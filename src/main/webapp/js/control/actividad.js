...
var control_actividad_list = function(path) {
    ...
    function loadDivView(contenido, place, id) {
        $(prefijo_div + place).empty().append((contenido)
                + '<button class="btn btn-primary" id="volver">Volver</button>');
        $(prefijo_div + '#volver').click(function() {
            $(prefijo_div + place).empty();
            linkBack();
        });
    }

    function loadDivForm(view, place, id, action) {
        cabecera = '';
        ...
        loadDivView(view.getEmptyForm(), place, id);
    ...
    return {
        viewRegister: function(view, id) {
            $(prefijo_div + "#header-list").empty();
            loadDivView(view.getObjectTable(id), '#datos', id);
        },
        newRegister: function(view, id) {
            $(prefijo_div + "#header-list").empty();
            loadDivForm(view, '#datos', id, "new");
        },        
        editRegister: function(view, id) {
            $(prefijo_div + "#header-list").empty();
            loadDivForm(view, '#datos', id, "edit");
        },
        inicia: function(view, pag, order, ordervalue, rpp, filter, filteroperator, filtervalue, callback, systemfilter, systemfilteroperator, systemfiltervalue) {
            ...
            if (callback) {
                ...
                $(prefijo_div + '#crear').click(function() {
                    ...
                    $(prefijo_div + "#header-list").empty();
                    loadDivForm(view, '#datos', $(this).attr('id'), "new");
                });
            }
