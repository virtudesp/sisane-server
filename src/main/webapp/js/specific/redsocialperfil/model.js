...
redsocialperfilModel.prototype.agregarAmigo = function (id) {
    $.when(ajax().ajaxCallSync('/ausiasYield2014/json?ob=amigo&op=agregaramigo&id=' + id, 'GET', '')).done(function (data) {
        feedback = data;
    });
    return feedback;
};
redsocialperfilModel.prototype.removeAmigo = function (id) {
    $.when(ajax().ajaxCallSync('/ausiasYield2014/json?ob=amigo&op=removeamigo&id=' + id, 'GET', '')).done(function (data) {
        feedback = data;                           
    });
    return feedback;
};
redsocialperfilModel.prototype.existeAmigo = function (id) {
    $.when(ajax().ajaxCallSync('/ausiasYield2014/json?ob=amigo&op=existeamigo&id=' + id, 'GET', '')).done(function (data) {
        feedback = data;                           
    });
    return feedback;
};
