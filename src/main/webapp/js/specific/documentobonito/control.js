...
var documentobonitoControl = function (strClase) {
    this.clase = strClase;
};
documentobonitoControl.prototype = new control('documentobonito');
documentobonitoControl.prototype.getClassNameDocumentobonito = function () {
    return this.getClassName() + "Control";
};
var oDocumentobonitoControl = new documentobonitoControl('documentobonito');


documentobonitoControl.prototype.listCuadros = function (place, objParams, callback, oModel, oView) {
    var thisObject = this;
    objParams = param().validateUrlObjectParameters(objParams);
    //get all data from server in one http call and store it in cache
    var oDocumentobonitoModel = oModel;
    oDocumentobonitoModel.loadAggregateViewSome(objParams);
    ...
    oView.populateSelectVisibleFieldsBox($('#selectVisibleFields'), oDocumentobonitoModel.getCachedCountFields());
    ...
    //show the table
    var fieldNames = oDocumentobonitoModel.getCachedFieldNames();
    if (fieldNames.length < objParams["vf"]) {
        objParams["vf"] = fieldNames.length;
    }
    if (callback) {
        var maximo = Math.max(oDocumentobonitoModel.getCachedCountFields(), 3);
        $("#selectVisibleFields").val(maximo);
    } else {
        $("#selectVisibleFields").val(objParams["vf"]);
    }
    var prettyFieldNames = oDocumentobonitoModel.getCachedPrettyFieldNames();
    ...
    var page = oDocumentobonitoModel.getCachedPage();
    if (parseInt(objParams["page"]) > parseInt(oDocumentobonitoModel.getCachedPages())) {
        objParams["page"] = parseInt(oDocumentobonitoModel.getCachedPages());
    }
    $("#pagination").empty().append(oView.getSpinner()).html(oView.getPageLinks(url, parseInt(objParams["page"]), parseInt(oDocumentobonitoModel.getCachedPages()), 2));
    ...
    $("#tableBody").empty().append(oView.getSpinner()).html(function () {
        return oView.getBodyPageTable(page, fieldNames, parseInt(objParams["vf"]), function (id) {
            ...
            //mejor pasar documentobonito como parametro y crear un repo global de código personalizado
        });
    });
    //show information about the query
    $("#registers").empty().append(oView.getSpinner()).html(oView.getRegistersInfo(oDocumentobonitoModel.getCachedRegisters()));
